...
func TestScore(t *testing.T) {
	...
	if s := Score(rn, []string{"os=linux", "is_container=true"}); s < 0 {
		t.Fatalf("should match, score=%d", s)
	}
	if s := Score(rn, []string{"os=windows"}); s >= 0 {
		t.Fatalf("should NOT match windows, score=%d", s)
	}
}

func TestToolchainMapping(t *testing.T) {
	if ToolchainImage("go") == "" {
		t.Fatal("go mapping empty")
	}
	// no container + toolchain -> default image
	j := &Job{RunsOn: []string{"os=linux", "toolchain=go"}}
	if img := ImageFor(j, &Runner{}); img != Toolchains["go"] {
		t.Fatalf("img=%q", img)
	}
	// explicit container overrides
	j2 := &Job{RunsOn: []string{"os=linux", "toolchain=go"}, Container: "my:latest"}
	if img := ImageFor(j2, &Runner{}); img != "my:latest" {
		t.Fatalf("override img=%q", img)
	}
}
...
func TestMatchExcludesSessionBound(t *testing.T) {
	...
	if m == nil || m.ID != "ci1" {
		t.Fatalf("matched %+v (should be ci1 only)", m)
	}
}
...
func TestSchedulerNeeds(t *testing.T) {
	...
	if run.Jobs[0].State != StateSuccess || run.Jobs[1].State != StateSuccess {
		t.Fatal("both must succeed")
	}
}

func TestSchedulerFailureStops(t *testing.T) {
	reg := NewRunnerRegistry()
	reg.Register(&Runner{ID: "h", Labels: []string{"os=linux"}})
	...
	if run.State != StateFailure {
		t.Fatalf("state=%s", run.State)
	}
	if run.Jobs[1].State != StateFailure {
		t.Fatal("b must be failure")
	}
}
