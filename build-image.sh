#!/usr/bin/env bash
# Build and push the ext extension image WITHOUT a local build daemon.
# Mirrors easylab/agent build-image.sh: in-cluster buildkitd -> docker
# archive -> skopeo -> forgejo OCI. Set NAME (ops-extension / repo-extension).
set -euo pipefail
DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

REGISTRY="${REGISTRY:-forgejo.develop.10.199.64.20.nip.io}"
NAMESPACE="${NAMESPACE:-easylab}"
NAME="${NAME:-ops-extension}"
